...
def Carryoverfileread(files,Detectionplatform,reportinfo,project,platform,manufacturers,Carryover2_True,Unit,digits,ZP_Method_precursor_ion,ZP_Method_product_ion,normAB):
    ...
    if Carryover2_True==0:
        ...
        for value in Carryover_dict.values():
            ...
            value.append(C1mean)
            value.append(C3mean)
            num=new_round(abs(float(C3mean)-float(C1mean))/float(C1mean)*100,1) #判断携带效应偏差，大于20%需要在后面备注不通过
            if float(num)>20:
                Carryover_judgenum+=1
                value.append(str(num)+" (不通过！)")
            else:
...
def related_Carryover(id): 
    ...
    Detectionplatform=ReportInfo.objects.get(id=id).Detectionplatform

    # 优先查找特殊参数设置里是否有数据，如有就调用，没有则调用通用性参数设置里的数据
    # 特殊数据抓取
    Carryover_special = Special.objects.get(project=project)   
    carryover_special = Carryoverspecial.objects.get(special=Carryover_special) 
    textlist_special = [] #特殊参数设置描述性内容
    if Carryoverspecialtexts.objects.filter(carryoverspecial=carryover_special).count()>0: 
        text_special = Carryoverspecialtexts.objects.filter(carryoverspecial=carryover_special)     
        for i in text_special:
            textlist_special.append(i.text)
    
    # 通用数据抓取
    Carryover_general = General.objects.get(name="通用性项目") #通用参数设置描述性内容
    carryover_general = Carryovergeneral.objects.get(general=Carryover_general)
    text_general = Carryovergeneraltexts.objects.filter(carryovergeneral=carryover_general)   
    textlist_general = []
    ...
    if Detectionplatform!="元素":
        ...
        dataCarryover = Carryover.objects.filter(reportinfo_id=id)

        if dataCarryover:
            Carryover_endreport_dict={}  #最终需要的字典  
            Carryover_endreport_norm=[]
            for i in dataCarryover:
                Carryover_endreport_norm.append(i.norm)
            
            Carryover_endreport_norm_distinct=[]
            for i in Carryover_endreport_norm:
                if i not in Carryover_endreport_norm_distinct:
                    Carryover_endreport_norm_distinct.append(i)

            for i in Carryover_endreport_norm_distinct:
                Carryover_endreport_norm=[]
                dataCarryover_group = Carryover.objects.filter(reportinfo_id=id,norm=i)
                for item in dataCarryover_group:
                    # 没有为每个化合物单独设置有效位数，则调用通用性设置
                    if Digitsdict=={} or list(Digitsdict.values())[0]==None: 
                        # group=[]
                        Carryover_endreport_norm.append(item.C1_1)
                        Carryover_endreport_norm.append(item.C2_1)
                        Carryover_endreport_norm.append(item.C3_1)
                        Carryover_endreport_norm.append(item.C1_2)
                        Carryover_endreport_norm.append(item.C2_2)
                        Carryover_endreport_norm.append(item.C3_2)
                        Carryover_endreport_norm.append(item.C1_3)
                        Carryover_endreport_norm.append(item.C2_3)
                        Carryover_endreport_norm.append(item.C3_3)
                        Carryover_endreport_norm.append(item.C1mean)
                        Carryover_endreport_norm.append(item.C3mean)
                        Carryover_endreport_norm.append(new_round(item.bias,2))
                    #为每个化合物单独设置了有效位数，则调用每个化合物的设置
                    else:
                        # Carryover_endreport_norm=[]
                        Carryover_endreport_norm.append(effectnum(item.C1_1,Digitsdict[i]))
                        Carryover_endreport_norm.append(effectnum(item.C2_1,Digitsdict[i]))
                        Carryover_endreport_norm.append(effectnum(item.C3_1,Digitsdict[i]))
                        Carryover_endreport_norm.append(effectnum(item.C1_2,Digitsdict[i]))
                        Carryover_endreport_norm.append(effectnum(item.C2_2,Digitsdict[i]))
                        Carryover_endreport_norm.append(effectnum(item.C3_2,Digitsdict[i]))
                        Carryover_endreport_norm.append(effectnum(item.C1_3,Digitsdict[i]))
                        Carryover_endreport_norm.append(effectnum(item.C2_3,Digitsdict[i]))
                        Carryover_endreport_norm.append(effectnum(item.C3_3,Digitsdict[i]))
                        Carryover_endreport_norm.append(item.C1mean)
                        Carryover_endreport_norm.append(item.C3mean)
                        Carryover_endreport_norm.append(new_round(item.bias,2))
                    # Carryover_endreport_norm.append(group)
                Carryover_endreport_dict[i]=Carryover_endreport_norm
        
            Carryover_conclusion="(C3mean-C1mean)/C1mean均小于20%，说明系统无携带效应，满足检测要求。"

            #每行最多排列7个化合物，如超过7个，需拆分表格
            d=Carryover_endreport_dict
            print(d)
            if len(d)<=7:
                d_reshape={}
                d_reshape["table1"]=d
                ...
                # elif len(d)==22 or len(d)==25: 
                #     d_reshape["table1"]=dict(list(d.items())[:len(d)//4+1])
                #     d_reshape["table2"]=dict(list(d.items())[len(d)//3+1:len(d)//3*2+1])
                #     d_reshape["table3"]=dict(list(d.items())[len(d)//3*2+1:]) 
                # elif len(d)==17 or len(d)==20: 
                #     d_reshape["table1"]=dict(list(d.items())[:len(d)//3+1])
            ...
            if len(textlist_special)!=0:
                return {"d_reshape":d_reshape,"textlist":textlist_special,"serial":len(textlist_special)+1,
                "Carryover_conclusion":Carryover_conclusion,"colspan_num":colspan_num}
            else:
                return {"d_reshape":d_reshape,"textlist":textlist_general,"serial":len(textlist_general)+1,
                "Carryover_conclusion":Carryover_conclusion,"colspan_num":colspan_num}
        ...
        if dataCarryover:
            ...
            for item in dataCarryover:
                ...
                Carryover_endreport_dict[item.norm]=group

            Carryover_conclusion="X2-X1均小于3SD1，说明系统无携带效应，满足检测要求。"


            if len(textlist_special)!=0:
                return {"Carryover_endreport_dict":Carryover_endreport_dict,"textlist":textlist_special,"serial":len(textlist_special)+1,
                "Carryover_conclusion":Carryover_conclusion}
            else:
                return {"Carryover_endreport_dict":Carryover_endreport_dict,"textlist":textlist_general,"serial":len(textlist_general)+1,
                "Carryover_conclusion":Carryover_conclusion}